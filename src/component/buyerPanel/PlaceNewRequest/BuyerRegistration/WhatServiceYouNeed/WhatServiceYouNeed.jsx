...
const WhatServiceYouNeed = ({
  ...
  const [loading, setLoading] = useState(false);
  const [checkingPostcode, setCheckingPostcode] = useState(false);
  const [showConfirmModal, setShowConfirmModal] = useState(false); // ✅ modal control
  ...
  const { citySerach, buyerRequest } = useSelector((state) => state.buyer);
  const dispatch = useDispatch();
  const inputRef = useRef(null);
  ...
  // ✅ “Leave” handler for modal
  const handleLeave = async () => {
    try {
      const formData = new FormData();
      formData.append("service_id", selectedService?.id || serviceId || "");
      formData.append("postcode", pincode || "");
      formData.append("city", citySerach || city || "");
      formData.append("form_status", 0);

      const res = await dispatch(registerQuoteCustomer(formData));
      if (res?.success) {
