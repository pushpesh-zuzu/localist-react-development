...
const AveragePriceTransportServices = ({
  title = "",
  avg_price='' ,
  RELTED_PRICE=[],
  ...
  return (
    <div className={styles.averagePriceContainer}>
      <h2 className={styles.heading}>
        Average cost of {isSingular?'a':''} <span className={styles.title}>{title}</span> {avg_price && <span>{avg_price}</span> }{" "}{showSpeicialits && 'specialists'} by region
        <span></span>
      </h2>
...
export default AveragePriceTransportServices;
